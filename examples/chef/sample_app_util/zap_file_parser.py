...
class ClusterType(TypedDict):
    ...
    attributes: Dict[int, AttributeType]


class EndpointType(TypedDict):
    _name: str
    client_clusters: Dict[int, ClusterType]
    server_clusters: Dict[int, ClusterType]
...
def generate_metadata(zap_file_path: str) -> List[Dict[str, EndpointType]]:
    ...
    for endpoint in app_data["endpointTypes"]:
        ...
        endpoint_obj: Dict[str, EndpointType] = {device_type_name: {}}
        return_obj.append(endpoint_obj)

        client_clusters: Dict[int, ClusterType] = {}
        server_clusters: Dict[int, ClusterType] = {}

        endpoint_obj[device_type_name]["client_clusters"] = client_clusters
        endpoint_obj[device_type_name]["server_clusters"] = server_clusters
        ...
        for cluster in endpoint["clusters"]:
            ...
            for attribute in cluster["attributes"]:
                if attribute["included"]:
                    attribute_obj: AttributeType = {
                        "name": attribute["name"], "value": _read_value(attribute["defaultValue"])}
            ...
            for command in cluster["commands"]:
                if cluster["side"] == "client" and command["outgoing"] == 1:
                    cluster_obj["commands"].append(command["name"])
            ...
            if cluster["side"] == "client":
                client_clusters[cluster["name"]] = cluster_obj
            else:
                server_clusters[cluster["name"]] = cluster_obj
